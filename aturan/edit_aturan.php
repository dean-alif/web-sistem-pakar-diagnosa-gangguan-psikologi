...
<html lang="en">
  <head>
    ...
    <!-- Website Title -->
    <title>Edit Aturan | Sistem Pakar Diagnosa</title>
    <!-- Favicon Website -->
  ...
  <body>
    ...
    <section class="admin pb-3">
      ...
      <div class="container p-3">
        ...
        <h5 class="fw-bold mt-4">Edit Aturan</h5>
        <div class="card bg-white text-dark p-3 col-sm-5 mx-auto">
          <form action="../assets/database/config_editAturan.php" method="post">
            <?php
              $kodeAturan = $_GET['kode_aturan'];
              $query = mysqli_query ($connect, "SELECT * FROM aturan WHERE kode_aturan = '$kodeAturan'") or die (mysqli_error ($connect));
              $aturan = mysqli_fetch_array ($query);
            ?>
            <div class="p-2">
              <label for="kode_aturan" class="form-label">Kode</label>
              <input type="text" class="form-control text-capitalize" name="kode_aturan" value="<?php echo $kodeAturan; ?>" autocomplete="off" required readonly/>
            </div>
            <div class="p-2">
              <label for="jika" class="form-label">Jika</label>
              <input type="text" class="form-control text-uppercase" name="jika" value="<?php echo $aturan['jika']; ?>" autocomplete="off" required />
            </div>
            <div class="p-2">
              <label for="maka" class="form-label">Maka</label>
              <input type="text" class="form-control text-uppercase" name="maka" value="<?php echo $aturan['maka']; ?>" autocomplete="off" required />
            </div>
